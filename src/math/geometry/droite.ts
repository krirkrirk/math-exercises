...
export abstract class DroiteConstructor {
  static fromTwoPoints(A: Point, B: Point, name = "d"): Droite {
    const a = new FractionNode(
      new SubstractNode(B.y, A.y),
      new SubstractNode(B.x, A.x),
    );
  ...
  static fromPointAndSlope(A: Point, m: Node, name = "d"): Droite {
    return new Droite(
      name,
      simplifyNode(m),
      simplifyNode(new SubstractNode(A.y, new MultiplyNode(m, A.x))),
    );
  }

  static fromPointAndAngle(A: Point, theta: Node, name = "d"): Droite {
    const m = new NumberNode(
      round(Math.tan(parseFloat(theta.toMathString())), 2),
    );
...
export class Droite {
  ...
  toPolynome(): string {
    const polynome = new Polynomial(
      [evaluate(this.b.toMathString()), evaluate(this.a.toMathString())],
      "x",
    );
