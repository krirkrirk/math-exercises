...
const getPropositions: QCMGenerator<QCMProps> = (n, { answer, isFromEvolutionToCM, evolution }) => {
  const propositions: Proposition[] = [];
  addValidProp(propositions, answer);

  if (isFromEvolutionToCM) {
    tryToAddWrongProp(propositions, (round(evolution / 100, 2) + '').replaceAll('.', ','));
    tryToAddWrongProp(propositions, evolution + '');
  } else {
    tryToAddWrongProp(propositions, '+' + (round(1 + evolution / 100, 2) * 100 + '\\%').replaceAll('.', ','));
  }
