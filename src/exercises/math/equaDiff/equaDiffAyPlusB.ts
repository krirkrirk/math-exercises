...
const getPropositions: QCMGenerator<Identifiers> = (n, { answer, a, b }) => {
  ...
  while (propositions.length < n) {
    ...
    tryToAddWrongProp(propositions,generatePropositions(randint(-10,11,[1,0,b]),b )[0].toTex());
  }
...
const generatePropositions  = (a: number,b: number) : EqualNode[] =>{
  ...
  let fraction = new MultiplyNode(new NumberNode(-1),new Rational(a,bB).simplify().toTree()).simplify();
  const y = new VariableNode('y');
  const c = new VariableNode('C');
  const aMultx = new ExpNode( new MultiplyNode(new NumberNode(a),new VariableNode("x")) );

  const firstProps = new EqualNode(
    y,
    new AddNode(
      new MultiplyNode(c, aMultx),
      fraction
    )
  );

  fraction = new MultiplyNode(new NumberNode(-1),new Rational(bB,a).simplify().toTree()).simplify();
  const secondProps = new EqualNode(
    y,
    new AddNode(
      new MultiplyNode( 
        c, 
        new ExpNode( new MultiplyNode(new NumberNode(bB),new VariableNode("x")) )
      ),
      fraction
    )
  );
...
const isAnswerValid: VEA<Identifiers> = (ans, {a, b})=>{
  let fraction = new MultiplyNode(new NumberNode(-1),new Rational(b,a).simplify().toTree()).simplify();
  ...
  const validAnswers = correctNode.toAllValidTexs({allowFractionToDecimal: true, allowRawRightChildAsSolution: true});
