...
const getPropositions: QCMGenerator<QCMProps> = (n, { answer, sqrt, randNbr }) => {
  ...
  if (sqrt === Math.floor(sqrt)) {
    ...
    const factor = higherFactor(randNbr);
    const radicand = randNbr / factor ** 2;
    tryToAddWrongProp(propositions, `${factor}\\sqrt{${radicand}}`);
    tryToAddWrongProp(propositions, `${radicand}\\sqrt{${factor}}`);
    tryToAddWrongProp(propositions, `${Math.floor(sqrt)}`);
    ...
    tryToAddWrongProp(propositions, `-\\sqrt{${-randNbr}}`);
    tryToAddWrongProp(propositions, `${Math.floor(Math.sqrt(-randNbr))}`);
    while (propositions.length < n) {
      const factor = randint(2, 5);
      const radicand = randint(2, -randNbr);
      tryToAddWrongProp(propositions, `\\{${factor}\\sqrt{${radicand}}\\ ; -${factor}\\sqrt{${radicand}} \\}`);
    }
  }
