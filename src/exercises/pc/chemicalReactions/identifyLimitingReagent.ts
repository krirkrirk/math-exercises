...
> = () => {
  ...
  deux réactifs $\\text{A} et \\text{B (en mol)}, sont $\\text{n}_{\\text{f}}(\\text{A}) = ${roundSignificant(
    firstReagentVariables[0],
    1,
  )} - ${
    firstReagentVariables[1]
  } \\text{x}_{\\text{max}}$ et $ ${roundSignificant(
    firstReagentVariables[0],
    1,
  )} - ${firstReagentVariables[1]}$. Quel est le réactif limitant ?`;
...
const getPropositions: QCMGenerator<Identifiers> = (n, { answer }) => {
  ...
  tryToAddWrongProp(propositions, "Le réactif A");
  tryToAddWrongProp(propositions, "Le réactif B");
