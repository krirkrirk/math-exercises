...
const solveExponentialEquationWithIC: QuestionGenerator<
  QCMProps,
  VEAProps
> = () => {
  ...
  const question: Question<QCMProps, VEAProps> = {
    ...
    answerFormat: "tex",
    qcmGeneratorProps: { answer, a, initialY },
  };
...
const isAnswerValid: VEA<VEAProps> = (ans, { a, initialY }) => {
  const solution =
    initialY === 0
      ? new NumberNode(0) // y(0) = 0 ==> y(x) = 0
      : new MultiplyNode(
...
export const exponentialDifferentialEquationWithIC: MathExercise<
  QCMProps,
  VEAProps
> = {
