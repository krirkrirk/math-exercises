...
const runServer = () => {
  ...
  app.post("/tableVea", jsonParser, (req: Request, res: Response) => {
    const exoId = req.query.exoId;
    ...
    if (!exo.isTableSVGAnswerValid) {
      res.send("No TableVEA implemented");
      return;
    }
    const result =
      exo.isTableSVGAnswerValid(ans as string[][], veaProps) ?? false;
    res.json({
      result,
    });
  });
